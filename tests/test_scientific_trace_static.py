...
class ScientificProductionTraceStaticTest(unittest.TestCase):
    ...
    def test_transport_moments_cancel_zero_mass_conditional_quotients(self) -> None:
        ...
        self.assertIn("keops_gaussian_reduction", readout)
    ...
    def test_implicit_sinkhorn_requires_forward_and_adjoint_convergence(self) -> None:
        ...
        self.assertIn("fugw_alpha", configuration)
    ...
    def test_topology_and_barrier_admissibility_are_hard_checks(self) -> None:
        ...
        self.assertNotIn("torch.cdist", topology)
        self.assertIn("return sliced_persistence_wasserstein", topology)
    ...
    def test_a800_concurrency_uses_useful_views_and_early_rank_binding(self) -> None:
        ...
        self.assertIn("checkpoint_iterations=True", barrier)
        self.assertIn("_regularize_degenerate_restoration_gradient", barrier)
        self.assertIn("norm_floor_squared = finfo.eps * reference_squared", barrier)
        ...
        self.assertIn("with torch.no_grad():", pipeline)
        self.assertIn("_materialize_feasible_stratum", pipeline)
    ...
    def test_a800_precision_and_mip_renderer_contracts_are_production_applied(self) -> None:
        ...
        self.assertIn("from gsplat.rendering import rasterization", renderer)
        self.assertIn("quats=quaternions.contiguous()", renderer)
        self.assertIn("scales=scales.contiguous()", renderer)
        self.assertIn("eps2d=self.contract.kernel_size", renderer)
        self.assertIn("packed=True", renderer)
        self.assertIn('rasterize_mode="antialiased"', renderer)
        self.assertIn("extra_signals=world_normals", renderer)
        self.assertIn("viewmats[:, :3, :] = extrinsics.contiguous()", renderer)
        self.assertNotIn("diff_gaussian_rasterization", renderer)
        self.assertIn("GsplatRenderer()", pipeline)
        ...
        self.assertIn("production gsplat renderer", selector)
